...
import * as ID3Frame from './id3frame.js'

export const parse = function(handle, callback) {
	const tags = {
			title: null,
			album: null,
			artist: null,
	...
	handle.read(128, handle.size - 128, function(err, buffer) {
		...
		if(buffer.byteLength !== 128 || dvHelper.getString(3, null, true) !== 'TAG') {
			processed.v1 = true;
			return process();
		}

		tags.v1.title = dvHelper.getString(30, 3).replace(/(^\s+|\s+$)/, '') || null;
		tags.v1.artist = dvHelper.getString(30, 33).replace(/(^\s+|\s+$)/, '') || null;
		tags.v1.album = dvHelper.getString(30, 63).replace(/(^\s+|\s+$)/, '') || null;
		tags.v1.year = dvHelper.getString(4, 93).replace(/(^\s+|\s+$)/, '') || null;

		/*
			* If there is a zero byte at [125], the comment is 28 bytes and the remaining 2 are [0, trackno]
			*/
		if(dv.getUint8(125) === 0) {
			tags.v1.comment = dvHelper.getString(28, 97).replace(/(^\s+|\s+$)/, '');
			tags.v1.version = 1.1;
			tags.v1.track = dv.getUint8(126);
		} else {
			tags.v1.comment = dvHelper.getString(30, 97).replace(/(^\s+|\s+$)/, '');
		}
		/*
	...
	handle.read(14, 0, function(err, buffer) {
		...
		/*
			...
			*/
		if(buffer.byteLength !== 14 || dvHelper.getString(3, null, true) !== 'ID3' || dv.getUint8(3) > 4) {
			processed.v2 = true;
			return process();
		}
		...
		/*
			...
			*/
		if((tagFlags & 0x40) !== 0) {
			headerSize += dvHelper.getUint32Synch(11);
		}

		/*
			* Calculate the tag size to be read
			*/
		tagSize += dvHelper.getUint32Synch(6);
		handle.read(tagSize, headerSize, function(err, buffer) {
			...
			while(position < buffer.byteLength) {
				...
				if(tags.v2.version[0] < 3) {
					slice = buffer.slice(position, position + 6 + dvHelper.getUint24(position + 3));
				} else {
					slice = buffer.slice(position, position + 10 + dvHelper.getUint32Synch(position + 4));
				}
